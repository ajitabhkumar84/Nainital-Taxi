-- ============================================================================
-- NAINITAL TAXI - ENHANCED DATABASE SCHEMA
-- ============================================================================
-- Features:
-- 1. 2-tier pricing (Off-Season, Season) with MULTIPLE season periods
-- 2. Booking blackout dates (block online booking for specific dates)
-- 3. 7 packages matching your actual offerings
-- 4. Flexible season management for long weekends
-- ============================================================================
...
-- ============================================================================
-- SEASONS TABLE (ENHANCED - Multiple periods for same season type)
-- ============================================================================
CREATE TABLE seasons (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  name TEXT NOT NULL,                     -- 'Off-Season' or 'Season'
  description TEXT,
  ...
  CONSTRAINT only_two_season_types CHECK (name IN ('Off-Season', 'Season'))
);
...
COMMENT ON TABLE seasons IS 'Only 2 season TYPES: Off-Season and Season. Can have multiple date ranges for each.';
COMMENT ON COLUMN seasons.priority IS 'When dates overlap, higher priority wins. Use for long weekends within off-season.';

-- ============================================================================
-- BOOKING BLACKOUT DATES (NEW!)
-- ============================================================================
CREATE TABLE booking_blackout (
  ...
  end_date DATE NOT NULL,
  reason TEXT,
  show_message TEXT DEFAULT 'Online booking unavailable. Please call or WhatsApp us.',
  is_active BOOLEAN DEFAULT TRUE,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE INDEX idx_blackout_dates ON booking_blackout(start_date, end_date);
CREATE INDEX idx_blackout_active ON booking_blackout(is_active) WHERE is_active = TRUE;

COMMENT ON TABLE booking_blackout IS 'Dates where online booking is disabled. Users can still see prices and contact info.';

-- ============================================================================
-- PRICING TABLE (Manual pricing per package-vehicle-season)
-- ============================================================================
CREATE TABLE pricing (
  ...
  -- Each package-vehicle can have 2 prices (off-season + season)
  UNIQUE(package_id, vehicle_type, season_name)
);

CREATE INDEX idx_pricing_package ON pricing(package_id);
CREATE INDEX idx_pricing_vehicle ON pricing(vehicle_type);
CREATE INDEX idx_pricing_season ON pricing(season_name);

COMMENT ON TABLE pricing IS 'Manual pricing: 2 prices per package-vehicle (off-season, season). References season_name, not season_id.';
...
CREATE TABLE bookings (
  ...
  season_name TEXT,  -- Changed from season_id
...
-- Function to get current season for a date
...
BEGIN
  ...
  SELECT name INTO season_record
  FROM seasons
  ...
  -- If found, return the season name, otherwise return 'Off-Season'
  IF FOUND THEN
    RETURN season_record.name;
  ELSE
    RETURN 'Off-Season';
  END IF;
END;
...
-- Function to get blackout message for a date
CREATE OR REPLACE FUNCTION get_blackout_message(check_date DATE)
RETURNS TEXT AS $$
DECLARE
  message TEXT;
BEGIN
  SELECT show_message INTO message
  FROM booking_blackout
  WHERE is_active = TRUE
    AND start_date <= check_date
    AND end_date >= check_date
...
ALTER TABLE booking_blackout ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Public read" ON booking_blackout FOR SELECT USING (is_active = TRUE);
