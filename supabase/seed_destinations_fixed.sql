-- ============================================================================
-- DESTINATION PAGES - ADDITIONAL SEED DATA (FIXED)
-- ============================================================================
-- Compatible with schema_final.sql (uses season_id instead of season_name)
...
BEGIN
  ...
  -- Get season IDs - Get the first off-season and first season
  -- You may need to adjust this logic based on your seasons table
  SELECT id INTO off_season_id FROM seasons WHERE name = 'Off-Season' LIMIT 1;
  SELECT id INTO peak_season_id FROM seasons WHERE name = 'Season' ORDER BY priority DESC LIMIT 1;
  ...
  IF off_season_id IS NULL THEN
    ...
    VALUES ('Off-Season', 'Year-round regular pricing', '2025-01-01', '2025-12-31', 0, TRUE)
    RETURNING id INTO off_season_id;
  END IF;

  IF peak_season_id IS NULL THEN
    INSERT INTO seasons (name, description, start_date, end_date, priority, is_active)
    VALUES ('Season', 'Summer peak season', '2025-04-15', '2025-06-30', 10, TRUE)
  ...
  RAISE NOTICE 'Successfully added pricing for all transfer packages';
END $$;
